            ...
            <div class="w-3/4 pl-8">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    foreach ($games as $g) {
                        ...
                        echo '<a href="?page=game&id=' . $g['id'] . '">';
                        echo '<img src="' . htmlspecialchars($g['image']) . '" alt="' . htmlspecialchars($g['name']) . '" class="w-full h-40 object-cover mb-4 rounded cursor-pointer hover:opacity-80 transition">';
                        echo '</a>';
                        echo '<h3 class="text-lg font-semibold">' . htmlspecialchars($g['name']) . '</h3>';
                        echo '<p class="text-sm text-gray-600">' . htmlspecialchars($g['info']) . '</p>';
                        echo '<p class="text-xl font-bold mt-2">€' . number_format($g['price'], 2) . '</p>';

                        if (!empty($g['availability'])) {
                            echo '<button class="add-to-cart-btn mt-4 px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600" data-game-id="' . $g['id'] . '" type="button">Ajouter au panier</button>';
                            echo '<span class="add-to-cart-feedback text-green-600 ml-2" style="display:none;"></span>';
                        } else {
